...
ADAPTER_DIR = "faranbutt789/guardpeft-qwen3-r16"
MAX_SEQ_LEN = 1024
MAX_NEW_TOK = 256
HARM_ALPHA = 12.0
...
def get_decoder_layers(model):
    ...
    if not hasattr(m, "layers"):
        raise AttributeError(
            ...
            f"Top-level attributes: {[n for n, _ in model.named_children()]}"
        )
    return m.layers
    ...
    def _collect(self, texts: List[str]) -> Dict[int, torch.Tensor]:
        ...
        for l in self.layers:
            if len(self._cache[l]) == 0:
                raise RuntimeError(
                    f"Cache empty for layer {l}. "
                    f"Hook was not triggered — check get_decoder_layers()."
                )
...
def build_contrastive_texts(
    df: pd.DataFrame, tokenizer, n: int = 100
) -> Tuple[List, List]:
...
def alpha_sweep(
    ...
    sweep_df.to_csv("./logs/alpha_sweep.csv", index=False)
    return sweep_df
...
if __name__ == "__main__":
    ...
    print("\nLoading Qwen3-1.7B + r16 adapter …")
    ...
    print("\n" + "=" * 60)
    print("PHASE 1 — Extract Contrastive Steering Vectors")
    print("=" * 60)
    ...
    torch.save({l: v for l, v in harm_vectors.items()}, "./steering/harm_vectors.pt")
    print("Steering vectors saved → ./steering/harm_vectors.pt")
    ...
    for ds_name, ds in [("Ethics", ethics_ds), ("PKU", pku_ds)]:
        ...
        res_base = run_inference(model, tokenizer, ds, f"r16_baseline_{ds_name}")
        res_base["dataset"] = ds_name
        all_results.append(res_base)

        print(f"\n── {ds_name}: Steered ──")
        res_steer = run_inference(
            model, tokenizer, ds, f"r16_steered_{ds_name}", best_hook
        )
        res_steer["dataset"] = ds_name
        all_results.append(res_steer)

    results_df = pd.DataFrame(all_results)
    results_df.to_csv("./logs/steering_eval.csv", index=False)

    print("\n" + "=" * 70)
    print("STEERING EVALUATION — BASELINE vs STEERED")
    print("=" * 70)
    cols = [
        "model",
        "dataset",
    ...
    p1 = mpatches.Patch(color=BASE_COLOR, label="Baseline r16")
    p2 = mpatches.Patch(color=STEER_COLOR, label="Steered r16")
    fig.legend(handles=[p1, p2], loc="upper right", fontsize=11)
    plt.suptitle(
        f"Activation Steering vs Baseline — harm_α={BEST_HARM_ALPHA}  reasoning_α={BEST_REASONING_ALPHA}",
        fontsize=13,
        fontweight="bold",
    )
    plt.tight_layout()
    plt.savefig("./outputs/steering_comparison.png", dpi=150, bbox_inches="tight")
    plt.show()
    print("\nPlot saved → ./outputs/steering_comparison.png")
